...
const navItems: Array<{ icon: string; label: string; key: string }> = [
...
function PlanningPage({ professorsCount }: { professorsCount: number }) {
...
function AddClassModal({ onClose, onAdd }: { onClose: () => void; onAdd: (cls: SchoolClass) => void }) {
...
function TeachersPage({
  ...
  return (
    <div style={S.pageWrap}>
      ...
      {profError ? (
        <div style={{ margin: '0 32px 12px', color: '#c0392b', fontSize: 13, fontWeight: 600 }}>
          {profError}
        </div>
      ) : null}
...
export default function App() {
  ...
  async function refreshAll() {
    ...
    setDbTime(data.dbStatus.dbTime)
    setDbVersion(data.dbStatus.dbVersion)
